...
def main() -> None:
    ...
    try:
        ...
        print(f"Failed to connect to ODrive: {exc}")
        sys.exit(1)
    ...
    print(
        f"Motor: {axis.config.motor.pole_pairs} pole pairs, "
        f"KT={axis.config.motor.torque_constant:.4f}"
    )
    print(
        f"Phase R: {axis.config.motor.phase_resistance:.4f} Ω, "
        f"L: {axis.config.motor.phase_inductance:.6f} H"
    )
    print(
        f"Encoder: RS485 AMT21 event-driven, "
        f"offset={axis.commutation_mapper.config.offset:.4f}"
    )
    print(
        "Thermistor: "
